...
describe("easy_read prompt", () => {
  it("documents the compatibility contract and restrained section context usage", () => {
    ...
    expect(prompt).toContain("Usa el texto completo de la seccion solo como contexto")
    expect(prompt).toContain("No copies informacion del contexto")
    expect(prompt).toContain("Cada salida debe corresponder solo al texto de entrada con el mismo indice")
    expect(prompt).toContain("No fusiones varios textos de entrada")
    expect(prompt).toContain("Usa ortografia correcta en el idioma de salida")
    expect(prompt).toContain("Si el texto de entrada enumera 3 o mas elementos como ejemplos, partes, propiedades, acciones o items")
    expect(prompt).toContain("No conviertas enumeraciones de ejemplos, partes, propiedades, acciones o items")
    expect(prompt).toContain("Usa saltos de linea dentro del string para separar la frase introductoria")
    expect(prompt).toContain("Si 3 o mas elementos son el sujeto de una sola frase corta y clara")
  })
})
